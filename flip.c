...
int main(int argc,char **argv){
    ...
    if (argc != 4)
    {
    fprintf (stderr, "usage: %s IMAGE IMAGE DST(.bmp) type_flip(1 pour horizental,2 pour vertical) \n", argv[0]);
    return EXIT_FAILURE;
    }
    ...
    if (atoi(n)== 1)
       flipType = SDL_FLIP_HORIZONTAL;
        ...
        flipType = SDL_FLIP_VERTICAL;
          ...
          printf("erreur \n");
    if(SDL_RenderCopyEx(renderer, texture, NULL, &rectangle,0,NULL,flipType)!= 0){
        SDL_DestroyRenderer(renderer);
        SDL_DestroyWindow(window);
